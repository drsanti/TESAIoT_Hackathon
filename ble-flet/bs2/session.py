...
class Bs2BleSession:
    ...
    async def connect_as_viewer(self, device: BLEDevice) -> None:
        """Sensor-node viewer: connect + CCCD + stream — never SENSOR_CFG_SET.

        Boot SENSOR_CFG on the MCU owns rates/masks. POLICY_SET 0x07 is a heal
        backup when firmware has not yet auto-armed TX_EVT from CCCD.
        """
        await self.connect(device, bootstrap="fast")
        await self.enable_streaming(refresh_cfg=False)

    async def go_live_on_existing_link(self) -> None:
        """Re-arm notify + stream on an already-connected link (heal / resume)."""
        await self.enable_streaming(refresh_cfg=False)
    ...
    async def enable_streaming(self, *, refresh_cfg: bool = False) -> None:
        ...
        if refresh_cfg:
            # Configurator path only — viewer mode never blocks on SENSOR_CFG GET.
            try:
        ...
        if cfg is not None:
            if not cfg.get("enabled") or cfg.get("mask", 0) == 0:
                if sample_key not in self._cfg_drop_logged:
                    ...
                    self._log(
                        "warn",
                        f"EVT dropped ({sample_key}): SENSOR_CFG disabled/mask=0 on device — "
                        f"configure via Bitstream Studio or reconnect",
                    )
                return
