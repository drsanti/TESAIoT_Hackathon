...
Desktop **BS2 over BLE** dashboard (Flet + bleak). This document describes how Auto connect, scene presets, and live EVT delivery work together.

## Overview

1. **JSON presets** (`bs2/sensor-scene-presets.v1.json`) define sensor rates, masks, and BMI270 mode.
2. After GATT connect, Auto applies the **`motion`** preset to firmware RAM over BLE.
3. Firmware publishes **`EVT_SENSOR`** according to that config.
4. The app decodes notifies and paints Live cards.

Preset writes are **RAM until reboot** (not flashed into the HEX image).

## Key source files

| Path | Role |
|------|------|
| `ui/app.py` | Auto FSM driver, scene buttons, go-live orchestration |
| `bs2/connection_fsm.py` | `ConnPhase` labels and hunt backoff constants |
| `bs2/session.py` | Bleak session: REQ writes, notify decode, scene apply |
| `bs2/scene_presets.py` | Load JSON → SENSOR_CFG / BMI270 wire payloads |
...
Default Auto scene: `AUTO_STREAM_SCENE_PRESET = "motion"`.
...
stateDiagram-v2
  ...
  LINKED --> LIVE: Stream on plus Motion scene
...
| **LINKED** | Connected; applying stream policy + scene |
