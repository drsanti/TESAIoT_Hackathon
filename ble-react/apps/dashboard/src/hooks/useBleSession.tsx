...
export function BleSessionProvider({ children }: { children: ReactNode }) {
  ...
  const [policyFlags, setPolicyFlags] = useState(0);
  ...
  const syncPolicy = useCallback((session: TbsBleSession) => {
    setPolicyFlags(session.getPolicyFlags());
  }, []);
  ...
  const setTransportMode = useCallback(
    (mode: BleTransportMode) => {
      ...
      void destroySession().then(() => {
        ...
        setLogs((prev) =>
          [
            `[info] Transport → ${mode === "bridge" ? "host Node bridge" : "Web Bluetooth"}`,
            ...prev,
          ].slice(0, 80),
        );
      });
  ...
  }, [destroySession, ensureSession, phase, syncPolicy, transportMode]);
  ...
  const ping = useCallback(
    async (...args: Parameters<TbsBleSession["ping"]>) => {
      const session = ensureSession();
      const res = await session.ping(...args);
      syncPolicy(session);
      return res;
    },
    [ensureSession, syncPolicy],
  ...
  const setBlePolicy = useCallback(
    async (...args: Parameters<TbsBleSession["setBlePolicy"]>) => {
      const session = ensureSession();
      const flags = await session.setBlePolicy(...args);
