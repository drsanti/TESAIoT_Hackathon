...
class BleDashboardApp:
    def __init__(self, page: ft.Page) -> None:
        ...
        self.last_preset_id = VIEWER_PRESET_LABEL
        self._hunt_fails = 0
    ...
    async def _connect_and_go_live(self, device: BLEDevice, *, source: str) -> bool:
        ...
        self.page.update()
        try:
            await self.session.connect_as_viewer(device)
        except Exception as exc:
            st = self.session.state
            ...
            if st.connected and (st.streaming or evt_flowing or (st.policy_flags & 0x02)):
                ...
                self._on_log(
                    "warn",
                    f"Viewer go-live incomplete ({exc!r}) — LIVE anyway "
                    f"(policy=0x{st.policy_flags:02x})",
                )
            else:
                self._on_log("error", f"Connect / viewer go-live failed ({source}): {exc!r}")
        ...
        st = self.session.state
        evt_flowing = sum(st.frame_raw.values()) > 0
        if not (st.streaming or evt_flowing or (st.policy_flags & 0x02)):
            self._on_log("error", f"Viewer go-live: no stream (policy=0x{st.policy_flags:02x})")
            if self.auto_enabled:
                self._set_phase(ConnPhase.HUNTING)
            else:
        ...
        self._on_log(
            ...
            f"Auto={self.auto_enabled} · cfg=device",
        )
    ...
    async def _go_live_existing(self) -> None:
        if not self.session.connected:
            return
        try:
            await self.session.go_live_on_existing_link()
            self.last_preset_id = VIEWER_PRESET_LABEL
            self.session.reset_frame_counts()
            self._set_phase(ConnPhase.LIVE)
            self._navigate("live")
            ...
            self._on_log(
                "info",
                f"LIVE (existing link) · streaming={self.session.state.streaming} · cfg=device",
            )
        except Exception as exc:
